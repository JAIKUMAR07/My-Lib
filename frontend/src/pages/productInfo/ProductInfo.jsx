...
const ProductInfo = () => {
  ...
  const product = useMemo(
    () =>
      books.find(
        (p) => String(p.id) === id || (!Number.isNaN(normalizedId) && p.id === normalizedId)
      ),
    [books, id, normalizedId]
  );
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          <div className="w-full lg:w-1/2 lg:pt-4 animate-slideIn">
            ...
            <div className="mb-6 flex items-center gap-2">
               <div className={`w-2 h-2 rounded-full ${product?.available_copies > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
               <span className={`text-xs font-black uppercase tracking-widest ${product?.available_copies > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {product?.available_copies > 0 ? `${product?.available_copies} Units Available` : 'Out of Stock / Reserved'}
               </span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
               <button 
                 disabled={product?.available_copies <= 0}
                 onClick={() => toast.success(`Request initiated for "${product?.title}". Please collect from the main desk.`, {
                   icon: '📚',
                   duration: 4000
                 })}
                 className={`w-full flex-1 py-5 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 ${
                   product?.available_copies > 0 
                   ? 'bg-slate-900 text-white shadow-slate-400 hover:bg-slate-800' 
                   : 'bg-slate-100 text-slate-400 cursor-not-allowed shadow-none'
                 }`}
               >
                  {product?.available_copies > 0 ? 'Borrow This Book' : 'Reservations Only'}
               </button>
               <button 
                onClick={() => isSaved ? deleteCart(product) : addCart(product)}
                className={`w-full sm:w-auto px-10 py-5 rounded-3xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 border-2 ${isSaved ? 'border-rose-100 text-rose-600 bg-rose-50/50' : 'border-slate-100 text-slate-600 hover:bg-slate-50'}`}
               >
                  {isSaved ? "Purge from Collection" : "Save for Later"}
               </button>
            </div>
