...
import { 
  ...
  Package, Scan, Bell, LogOut, ChevronRight, Activity, Shield
} from "lucide-react";
...
const LibrarianProfile = () => {
  ...
  const librarianData = {
    ...
    email: "librarian@nexlib.io",
    phone: "+1 (800) NEX-STAFF",
    staffId: "LIB-402-Q",
    address: "West Wing Hub, Archive Floor",
        ...
        <div className="absolute -bottom-20 left-6 flex items-end gap-6 md:left-12 md:gap-8">
          <div className="relative">
            <div className="h-32 w-32 rounded-3xl bg-white p-2 shadow-2xl md:h-44 md:w-44">
              <div className="flex h-full w-full items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-cyan-600">
      ...
      <div className="grid grid-cols-1 gap-8 px-5 pb-8 pt-24 md:px-12 lg:grid-cols-12 lg:gap-12 lg:pb-12 lg:pt-28">
        <div className="space-y-10 lg:col-span-8">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
            {stats.map((stat) => (
              <div key={stat.label} className="group rounded-[2rem] border border-slate-100 bg-slate-50 p-6 transition-all hover:border-emerald-200 hover:bg-white hover:shadow-2xl">
...
const ActionButton = ({ icon: Icon, label, primary, danger }) => (
  <button className={`w-full rounded-2xl px-6 py-4 text-[10px] font-black uppercase tracking-widest transition-all ${primary ? "bg-slate-900 text-white hover:bg-slate-800 shadow-xl" : danger ? "border border-rose-100 bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white" : "border border-slate-200 bg-white text-slate-500 hover:border-slate-400 hover:text-slate-900"}`}><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Icon className="h-4 w-4" />{label}</div><ChevronRight className="h-3 w-3 opacity-30" /></div></button>
);
